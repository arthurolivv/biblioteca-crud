...
function gerarExemplares() {
    const qtd = parseInt(document.getElementById('qtdExemplaresInput').value);
    ...
    for (let i = 0; i < qtd; i++) {
        ...
        exemplarDiv.innerHTML = `
            <div class="card-body">
                <h6 class="card-subtitle mb-3 text-muted">Exemplar ${i + 1}</h6>
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label">Código do Exemplar</label>
                        <input type="text" 
                               name="exemplares[${i}].codigo_exemplar" 
                               class="form-control" 
                               placeholder="Ex: EX-${String(i + 1).padStart(4, '0')}"
                               pattern="^EX-\\d+$"
                               required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Exclusivo para consulta local?</label>
                        <select name="exemplares[${i}].proprio" class="form-select" required>
